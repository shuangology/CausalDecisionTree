...
def find_best_question(Rows, question_excluded):
    ...
    maxAskCount = 2 # delete the questions that are asked twice
    ...
    if len(question_list) == 0:
        raise ValueError('Find Best Question: Rows is empty')
...
def print_tree(
    ...
    if isinstance(node, Leaf):
        print(str(height) + spacing + "Predict:", node.value, node.predictions,
              " GINI:", str(gini(node.predictions)), file=sourceFile)
        return
    if isinstance(node.real, Leaf):
        print(str(height) + spacing + "Predict:",
              node.real.value,
              node.real.predictions,
              " GINI:",
              str(gini(node.real.predictions)),
              file=sourceFile)
        return

    # Print the question at this node
    print(str(height) + spacing + str(node.question), file=sourceFile)

    # Call this function recursively on the true branch
    print(str(height) + spacing + '--> True:', file=sourceFile)
    print_tree(node.true_branch, height + 1, spacing + "  ", sourceFile)

    # Call this function recursively on the false branch
    print(str(height) + spacing + '--> False:', file=sourceFile)
    print_tree(node.false_branch, height + 1, spacing + "  ", sourceFile)
...
def _main():
    ...
    info_file_name = '../../data/CBBC_Monthly_Info/TOTALINFO_Update.msg.zip'
    archive = zipfile.ZipFile(info_file_name, 'r')
    info_df = pd.read_msgpack(
        archive.open('TOTALINFO_Update.msg'),
        encoding='utf-8')

    # only consider underlying HSI

    hsi_df = info_df[info_df['Underlying'] == 'HSI']
    hsi_df = hsi_df[hsi_df['Turnover'] > 0]

    if random_flag:
        import random
        random.seed(2021)
        target = hsi_df.loc[random.choices(
            list(info_df.index), k=random_pick), :]
    else:
        target = hsi_df

    print('Size of the sample{}'.format(len(target)))

    target['Moneyness_abs'] = np.abs(np.log(target['Strike_Level']/target['Underlying_Close']))

    Star_Issuer = ['CS', 'HT', 'JP', 'SG', 'UB']

    target['star_issuer'] = target['Issuer'].isin(Star_Issuer).apply(str)

    #
    # target = hsi_df.loc[random.choices(list(info_df.index), k=2000), :]
    ...
    filename = 'Day_NetTurn_tree' + '_sp_' + \
        str(args.sp) + '_h_' + str(args.hmax) +'_r_' + str(args.random) + '_n_'+str(var_num)+'.txt'
    PrintTreeFile = open(filename, 'w')
